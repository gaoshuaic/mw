 ...
 * output class name
 */
public class BudgetDateListUI extends AbstractBudgetDateListUI
{
    private static final Logger logger = CoreUIObject.getLogger(BudgetDateListUI.class);
    
    /**
     * output class constructor
     */
    public BudgetDateListUI() throws Exception
    {
        super();
    }
    @Override
	protected FilterInfo getDefaultFilterForQuery() {
         ...
         if("00000000-0000-0000-0000-000000000000CCE7AED4".equals(cuid)){
        	 ...
        	 String sql  = " SELECT FNAME_L2 as fname FROM  T_ORG_Company where FPARENTID  = '"+cuid+"'  ";
             String name = "";
                   ...
                   if(rs!=null && rs.size() > 0){
                           while(rs.next()){     
                                 if(rs.getObject("FNAME")!=null &&  !"".equals(rs.getObject("FNAME").toString())){
                                	 name = name+""+rs.getObject("FNAME").toString()+","; 
                                 }  
                           }
                   }  
                   name = name.substring(0, name.length()-1);
                   if(!flag){//只能查看自己的
                	   filter.getFilterItems().add(new FilterItemInfo("COMPANY.NAME",name,CompareType.INCLUDE ));  
                   }
             } catch (Exception e) {
                   e.printStackTrace();
             }
